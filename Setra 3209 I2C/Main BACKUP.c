...
//unsigned char ReadRegisters(unsigned char registerNumber, unsigned char *ptrData, unsigned char numBytes);
//unsigned char ReadRegistersByte(unsigned char registerNumber, unsigned char *ptrData);
//unsigned char WriteRegisterBlock(unsigned char registerNumber, unsigned char *ptrData, unsigned char numBytes);
//unsigned char WriteRegister(unsigned char registerNumber, unsigned char dataByte);
...
unsigned short i = 0;    
    ...
    for (i = 0; i < numBytes; i++) ptrDataOut[i] = i * 1;
...
unsigned char command[1] = {ACTIVATE_STORE};
    ...
    //WriteRegister(MEM_CTRL_REGISTER, ACTIVATE_STORE);
...
unsigned char command[1] =  {ACTIVATE_ERASE};
    ...
    //WriteRegister(MEM_CTRL_REGISTER, ACTIVATE_ERASE);
    PcapWriteNVRAM(MEM_CTRL_REGISTER + CONFIG_ADDRESS_OFFSET, command, 1);
...
unsigned char command[1] =  {ACTIVATE_RECALL};
    ...
    //WriteRegister(MEM_CTRL_REGISTER, ACTIVATE_RECALL);
    PcapWriteNVRAM(MEM_CTRL_REGISTER + CONFIG_ADDRESS_OFFSET, command, 1);
...
{       
    ...
    MasterWriteI2C(WRITE_CONFIGURATION); // Send opcode
    IdleI2C(); // Wait to complete
    if (I2CSTATbits.ACKSTAT) 
    {
        printf("\rNO ACK #2"); // Get ACK from EEPROM
    ...
    MasterWriteI2C(registerNumber | 0b11000000); // Send register number OR'd with two high bits
    IdleI2C(); // Wait to complete
    if (I2CSTATbits.ACKSTAT) 
    {
        printf("\rNO ACK #3"); // Get ACK from EEPROM
...
unsigned char ReadRegisterBlock(unsigned char registerNumber, unsigned char *ptrData, unsigned char numBytes)
{
    int i;    
    ...
    MasterWriteI2C(READ_CONFIGURATION); // Send opcode
    IdleI2C(); // Wait to complete
    if (I2CSTATbits.ACKSTAT) 
    {
        printf("\rNO ACK #2"); // Get ACK from EEPROM
...
{    
    ...
    *ptrData = I2CRCV;    // Read data byte from buffer
    ...
    I2CCONbits.ACKDT = 1;   // No ACK expected for one data byte
    I2CCONbits.ACKEN = 1;
